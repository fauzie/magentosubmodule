<?php
class KS_Megamenu_Block_Megamenu extends Mage_Page_Block_Html_Topmenu
{
	...
	protected function _getHtml(Varien_Data_Tree_Node $menuTree, $childrenWrapClass)
	{
		...
		foreach ($children as $child) {
			...
			if ($childLevel == 0 ) {
				...
				$childClass .= ' '. $category->getMegamenuType();
				$megamenuData['type'] = $category->getMegamenuType();
				$megamenuData['layout'] = $category->getMegamenuLayout();
				$megamenuData['menu'] = $category->getMegamenuMenu();
				$megamenuData['top'] = $category->getMegamenuTop();
				$megamenuData['bottom'] = $category->getMegamenuBottom();
				$megamenuData['right'] = $category->getMegamenuRight();
				$megamenuData['percent'] = $category->getMegamenuRightPercent();
	...
	protected function _drawMenuBlock($type, $content, $class = '')
	{
		...
		if ( !empty($content) ) {
			...
			$html .= $this->helper('megamenu')->processCmsBlock($content);
			$html .= '<div class="clear"></div>';
			$html .= '</div>';
		}
